...
As stated above, I don't want to release this as an "official" gem on RubyGems. For that reason, if you want to install this code as a gem, you can either use the <tt>gem</tt> command manually:

    gem build robot_remote_server
    gem install robot_remote_server-{version}.gem

Or you can install the project's dependencies (run <tt>bundle install</tt>) and use the rake tasks defined by the Echoe gem:
